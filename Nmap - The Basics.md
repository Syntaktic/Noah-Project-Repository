...
A: To find this out, firstly we will run an nmap list scan of the target IP address. To do this, we will enter the following command: ![alt text](<Images/nmap-fig1>)
After the scan has completed, it should look something like the following image: ![alt text](<Images/nmap-fig2>)
...
A: In order to do this, we must add an option specifically looking for ports that complete the three-way handshake. We do this with the -sT option shortcut in nmap. ![alt text](<Images/nmap-fig3>)
...
A: This part does not require the linux command line, but our web browser.  Through our previous nmap scan, notice that port 8080 is open. ![alt text](<Images/nmap-fig4>)
...
![alt text](<Images/nmap-fig7>)As seen in the figure above, port 8008 has now given us more insight into what service and version is running on the open port. 

**Task 5 - How Fast is Fast?**

*Q: What is the non-numeric equivalent of -T4?*

A: -T aggressive. This answer is given to us by the folks at tryhackme through this handy table. see the figure below for more details. ![alt text](<Images/nmap-fig7>)
